...
const Control = (props) => {
  ...
  return (
    <div className={classes.container}>
      ...
      {haveAccount && <button className={classes.btn}>Delete</button>}
      {haveAccount && <button className={classes.btn}>Update</button>}
      <button onClick={signOutHandler} className={`${classes.btn} ${classes.singout}`} >{haveAccount ? 'sing out' : 'back'}</button>
    </div>
  );
};
